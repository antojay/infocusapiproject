...
{
    internal sealed class Resend810FormOkButton : BoneItem
    {
        private static readonly ILog _logger = LogManager.GetLogger(typeof(Resend810FormOkButton));
        public Resend810FormOkButton()
            : base()
        {
                        ...
                        for (int ii = 1; ii <= noRows; ii++)
                        {
                            ...
                            if (bIsChecked)
                            {
                                ...
                                if (purchaseOrd.Trim().Length > 0)
                                {
                                    ...
                                    {
                                        sqlConnection.Open();
                                        using (SqlCommand command = new SqlCommand("UPDATE InfocusEdi850HeaderRecord set Processed810 = 0 where PurchaseOrderReference = '" + purchaseOrd.Trim() + "' ", sqlConnection))
                                        {
                                            command.ExecuteNonQuery();
                                            String msg = "Set purchase order " + purchaseOrd.Trim() + " to resend 810";
                                            _logger.Debug(msg);
